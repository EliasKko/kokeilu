...
// add your code here
const htmlElem = document.querySelector('#pictures')
for (let picture of picArray) {
  const Artcl = document.createElement('article');
  Artcl.class = 'card';
  const h = document.createElement('h2');
  h.innerHTML = `${picArray.title}`
  Artcl.appendChild(h)
  const figr = document.createElement('figure')
  const img = document.createElement('img')
  img.src = `${picture.image.medium}`
  img.alt = `${picArray.title}`
  const capt = document.createElement('figcaption')
  capt.innerHTML = `${picArray.caption}`
  figr.appendChild(img)
  figr.appendChild(capt)
  Artcl.appendChild(figr)
  const p = document.createElement('p')
  p.innerHTML = `${picArray.description}`
  Artcl.appendChild(p)
